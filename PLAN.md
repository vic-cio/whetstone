...
- **Lesson prose is parsed to data, never to HTML.** Lesson text is written by an agent and rendered in the host window, which holds the only bridge to the main process, so `<script>` in a Lesson comes out as characters and a link the app would not open stays as text. This cost tables and maths, which arrived in phase 7 once the first real Course made the case: it was a course on the Fourier transform and its prose was full of LaTeX. Maths is KaTeX, bundled with its fonts so an equation renders offline, given the expression as a string with `trust` off. Tables are the usual pipe form. Both are prose rather than blocks of their own, so they work in a callout, a caption, a Task's prompt, an option and an explanation, all of which turned out not to be running through the inline parser at all.
